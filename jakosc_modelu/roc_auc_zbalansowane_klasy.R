...
dane_y_ones = dane[dane$y == 1, ]
rownames(dane_y_ones) = NULL
dane_y_zeros = dane[dane$y == 0, ]
rownames(dane_y_zeros) = NULL

num_train = round(nrow(dane_y_zeros)/2)

set.seed(1234)
ones_sample = sample(1:nrow(dane_y_ones), num_train)
zeros_sample = sample(1:nrow(dane_y_zeros), num_train)

daneTrain = as.data.frame(rbind(dane_y_ones[ones_sample,], dane_y_zeros[zeros_sample,]))
daneTest = as.data.frame(rbind(dane_y_ones[-ones_sample,], dane_y_zeros[-zeros_sample,]))
