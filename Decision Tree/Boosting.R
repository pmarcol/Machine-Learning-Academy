...
cp_list = c(0.2, 0.05, 0.01)
minsplit_list = c(1, 6, 12)
for(ntrees_val in ntrees_list){  
  for(cp_val in cp_list){
    for(minsplit_val in minsplit_list){
      ...
      set.seed(123)
      boosting_classifier = boosting.cv(formula = y~.,
                                      data = dane_spirals_train,
                                      v=3,
                                      mfinal = ntrees_val,
                                      control = rpart.control(cp = cp_val,
                                                              minsplit = minsplit_val))
      if(boosting_classifier$error < best_params[1]){
        best_params = c(boosting_classifier$error, ntrees_val, cp_val, minsplit_val)
      }
    }
  }
}
